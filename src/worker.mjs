...
export default {
  async fetch (request) {
    ...
    try {
	  ...
	  if (isEdge) {
		pathname = pathname.substring("/edge".length);
	  }

      switch (true) {
        case pathname.startsWith("/gemini"): {
          const forwardPath = pathname.substring("/gemini".length);
          const newUrl = `${BASE_URL}${isEdge ? "/edge" : ""}${forwardPath}${url.search}`;
...
const adjustProps = (schemaPart) => {
  ...
  if (Array.isArray(schemaPart)) {
    ...
    // 无论条件如何，都直接删除 additionalProperties 字段
